...
execute "after-deploy-resources" do
  ...
  unless node[:_after_deploy_resources].nil?
    node[:_after_deploy_resources].each do |resource, action|
      notifies action, resource
    end
  end

  not_if do
    node[:_after_deploy_resources].nil? || node[:_after_deploy_resources].empty?
  end
end
