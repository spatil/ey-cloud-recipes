    ...
    execute "generate-self-signed-cert" do
      command %Q{
            openssl req -x509 -days 365 -newkey rsa:1024 -nodes \
              ...
              -keyout /data/apache2/ssl/#{app}.key -out /data/apache2/ssl/#{app}.crt
          }
      creates "/data/apache2/ssl/#{app}.key"
    end
...
execute "add-apache-to-default-runlevel" do
  command "rc-update add apache2 default"
  action :run
  not_if "rc-status | grep apache2"
end
